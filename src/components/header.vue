<template>
    <Disclosure as="nav" class="bg-[#24103d]" v-slot="{ open }">
        ...
        <div class="flex flex-1 items-center justify-center sm:items-stretch sm:justify-between">
          <div class="flex shrink-0 items-center">
            <router-link to="/" class="flex items-center">
                ...
                <h1 class="text-2xl ml-2 text-white">VueBlog</h1>
            </router-link>
          </div>
          <div class="hidden sm:mr-6 sm:block">
            <div class="flex space-x-4">
              <router-link v-for="item in navigation" :key="item.name" :to="item.href" class="text-[#ffffff]">{{ item.name }}</router-link>
            </div>
          </div>
        </div>
